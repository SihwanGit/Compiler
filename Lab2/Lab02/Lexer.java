...
public class Lexer {
    ...
    private char getchar() { // Return next char
        int c = 0;
        try { 
            c = input.read(); //한 캐릭터씩 버퍼를 읽어 리턴하는 함수 input.read()
            if (c == -1) 
                c = eofCh;
    ...
    public Token getToken( ) { // Return next token
        do {
            if (Character.isLetter(ch)) { // ident or keyword
                ...
                return Token.idORkeyword(s); //char인 경우
            } 
            ...
            if (Character.isDigit(ch)) { // number 
                ...
                return Token.NUMBER.setValue(s); //숫자인 경우. 숫자가 나올 떄까지 캐릭터를 받아 스트링 형태로 만든 후 반환
            } 

	        switch (ch) { //특수문자는 switch-case문으로 정의
            case ' ': case '\t': case '\r': 
                ch = getchar();
                break;

	        case eolnCh: //공백 혹은 개행문자는 무시
		        ch = getchar();
			    if (ch == '\r')			// for Windows
